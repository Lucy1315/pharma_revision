"""
프로테조밉주3.5mg 원시자료 분석 엑셀 생성기
모든 원시자료 파일을 읽어 분석 가능한 엑셀 파일로 통합
"""
import sys
...
OUTPUT = Path(__file__).parent / "data" / "output" / "프로테조밉주3.5mg_원시자료분석.xlsx"
DRUG_CODE = "201506668"
DRUG_NAME = "프로테조밉주3.5mg"
...
# ═══════════════════════════════════════════════════════════
def load_data():
    demo = read_txt("DEMO.txt")
    drug = read_txt("DRUG.txt")
    event = read_txt("EVENT.txt")
    assessment = read_txt("ASSESSMENT.txt")
    drug1 = read_txt("DRUG1.txt")
    drug2 = read_txt("DRUG2.txt")
    drug3 = read_txt("DRUG3.txt")
...
def build_merged(demo, drug, event, assessment):
    """대상 의약품 기준 통합 데이터프레임"""
    target_kaers = drug[
        (drug["DRUG_CD"] == DRUG_CODE) & (drug["DRUG_GB"].isin(["1", "2", "3"]))
    ]["KAERS_NO"].unique()
    ...
    drug_target = drug_t[drug_t["DRUG_CD"] == DRUG_CODE][["KAERS_NO", "DRUG_SEQ", "DRUG_GB_NM", "ACTION_NM"]].copy()
    ...
    drug_prio = drug_t[drug_t["DRUG_CD"] == DRUG_CODE][["KAERS_NO", "DRUG_SEQ", "DRUG_GB"]].copy()
...
def write_summary_sheet(wb, demo, drug, event, merged):
    ...
    row = 1
    # 제목
    c = ws.cell(row=row, column=2, value=f"■ {DRUG_NAME} 원시자료 분석 요약")
...
def write_raw_drug(wb, drug, drug1, drug2, drug3):
    ...
    # 대상 의약품 행 강조
    target_mask = merged["DRUG_CD"] == DRUG_CODE

    write_header_row(ws, 1, [""] + names)
    ws.row_dimensions[1].height = 35

    for ri, (_, row_data) in enumerate(merged[cols].iterrows()):
        is_target = (merged.iloc[ri]["DRUG_CD"] == DRUG_CODE)
        alt = ri % 2 == 1
...
def main():
    print("데이터 로드 중...")
    demo, drug, event, assessment, drug1, drug2, drug3 = load_data()
    merged = build_merged(demo, drug, event, assessment)
    ...
    print("요약 통계 시트 작성 중...")
    write_summary_sheet(wb, demo, drug, event, merged)

    print("분석 테이블 시트 작성 중...")
    write_analysis_tables(wb, merged, demo)

    print("Line Listing 시트 작성 중...")
    write_line_listing(wb, merged)

    print("원시자료 시트 작성 중...")
    write_raw_demo(wb, demo)
    write_raw_drug(wb, drug, drug1, drug2, drug3)
