...
with st.expander("📌 사용 방법", expanded=False):
    ...
    2. **제품 정보 조회** — 다음 둘 중 선택 (공공데이터포털 API 사용):
       - **품목기준코드로 조회**: 숫자 코드(예: `201506668`) 입력 시 상세정보 자동 조회
       - **제품명으로 검색**: 제품명(일부 가능) 입력 → 검색 결과 중 해당 품목 선택
       - 조회 성공 시 하단에 [nedrug 확인 링크](https://nedrug.mfds.go.kr)가 노출되어 브라우저에서 재확인 가능
    3. **분석 기간**은 업로드된 DEMO.txt에서 자동 감지, 필요 시 직접 수정
    4. **제품 정보 확인/수정** 칸의 자동 입력 값 검토 후 **보고서 생성** 클릭
    5. 생성된 **원시자료 분석 엑셀** 및 **안전관리보고서 Word** 다운로드
    6. Word 초안의 노란색 항목을 직접 수정하여 최종화
