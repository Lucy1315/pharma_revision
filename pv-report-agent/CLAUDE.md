...
├── .claude/
│   ├── rules/domain.md      ← 도메인 규칙 (WHOART SOC · 검토 마커 · 텍스트 규칙)
│   └── plans/BACKLOG.md     ← 후속 작업 단일 진입점 (P0/P1/P2/Low)
├── data/
...
│   ├── learnings/           ← 배포·운영·외부 API 교훈 (왜 그렇게 했는가)
...
│   ├── aggregator.py        ← Word/Excel 공유 집계 (단일 진입점)
│   ├── excel_builder.py     ← 8~9시트 엑셀 생성
│   ├── report_builder.py    ← Word 문서 생성
│   └── product_scraper.py   ← 공공데이터포털 API + nedrug fallback
├── tests/                   ← pytest (74개, 2026-04-18 기준)
├── scripts/                 ← ../scripts/pre-push 등 개발 자동화
...
1. **후속 작업**: `.claude/plans/BACKLOG.md` — 지금 뭘 해야 하는가
2. **도메인 룰**: `.claude/rules/domain.md` — 절대 바꾸면 안 되는 비즈니스 규칙
3. **부딪힌 교훈**: `docs/learnings/` — 같은 실수 반복하지 않도록
